...
async function resolveRegionsPath(forWrite = false) {
  if (!forWrite) {
    try {
      await fs.access(LOCAL_REGIONS_PATH)
      return LOCAL_REGIONS_PATH
    } catch {
      return TMP_REGIONS_PATH
    }
  }
...
async function readLocalRegions() {
  const dataPath = await resolveRegionsPath(false)
  try {
...
async function writeLocalRegions(regions) {
  const dataPath = await resolveRegionsPath(true)
  await ensureDataFile(dataPath)
