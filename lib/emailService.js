...
class EmailService {
  ...
  async generateAIContent(type, data) {
    ...
    // Build only the requested prompt. These used to be plain object-literal
    // values, which meant every template (including 'property-alert', which
    // reads data.properties.length) was evaluated on every call regardless of
    // `type` -- crashing with "Cannot read properties of undefined" whenever
    // welcome/inquiry-response were requested, since `data.properties` isn't
    // passed for those.
    const promptBuilders = {
      welcome: (d) => `Write a concise welcome email for ${d.userName}. Return JSON with keys subject and body.`,
      'inquiry-response': (d) =>
        `Write a concise inquiry confirmation email for ${d.userName} about "${d.propertyTitle}". Return JSON with keys subject and body.`,
      'property-alert': (d) =>
        `Write a concise property alert email for ${d.userName}. We found ${d.properties?.length ?? 0} properties. Return JSON with keys subject and body.`
    };
  ...
  async sendEmail({ to, subject, text, html, replyTo }) {
    if (!this.isConfigured) {
      ...
      console.log('========================================');
      console.log('From (System):', process.env.RESEND_FROM_EMAIL || '(Not Set)');
      console.log('To:', to);
    ...
    if (!verifiedSender) {
      const errorMsg =
        'Missing RESEND_FROM_EMAIL environment variable. Resend requires a verified sender.';
    ...
    try {
      ...
      if (error) {
        console.error('[EMAIL SYSTEM] Error sending email via Resend:', error.message || error);
        return {
          success: false,
          provider: 'resend',
      ...
      console.error('[EMAIL SYSTEM] Error sending email via Resend:', error.message);
      return {
        success: false,
        provider: 'resend',
        error: error.message
  ...
  // Rejects unset values and obvious unfilled template placeholders (e.g. a
  // deploy that copied env.template without editing it) so a lead notification
  // never gets silently "sent" to a domain nobody owns.
  isPlaceholderAdminEmail(value) {
    const normalized = String(value || '').trim().toLowerCase();
    if (!normalized) return true;
    return /@(yourdomain\.com|example\.com|domain\.com)$/.test(normalized);
  }
  ...
  async sendAdminInquiryNotification({ type = 'property', name, email, phone, message, propertyTitle }) {
    ...
    if (this.isPlaceholderAdminEmail(adminEmail)) {
      const reason = !adminEmail
        ? 'ADMIN_NOTIFY_EMAIL is not configured'
        : `ADMIN_NOTIFY_EMAIL is still set to an unfilled placeholder ("${adminEmail}"); set it to a real inbox`;
      console.error(`[EMAIL SYSTEM] ${reason}`);
      return {
        success: false,
        provider: 'none',
        error: reason
  ...
  // Client-facing reply written by the admin. Copy is Czech to match the rest
  // of the client emails. `propertyTitle` must be a human-readable title (never
  // a listing slug/ID) -- when it's missing we use a generic subject instead.
  async sendInquiryResponse({ userEmail, userName, propertyTitle, inquiryMessage, responseMessage }) {
    const safeTitle = this.sanitizeHeaderValue(propertyTitle);
    const subject = safeTitle
      ? `Odpověď na vaši poptávku: ${safeTitle}`
      : 'Odpověď na vaši poptávku';
    const intro = `Dobrý den ${userName || 'zájemce'},`;
    const body = responseMessage;
    const closing = 'S pozdravem,\nDomy v Itálii';
    ...
    return this.sendEmail({
      ...
      text: this.formatPlainText({
        intro,
        body,
        bullets: inquiryMessage ? [`Vaše původní zpráva: ${inquiryMessage}`] : [],
        closing
      }),
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #0f172a; line-height: 1.6;">
          <p style="font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: #64748b; margin-bottom: 8px;">Odpověď na poptávku</p>
          ...
          ${inquiryMessage ? `<div style="margin: 24px 0; padding: 14px 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px;"><strong>Vaše původní zpráva:</strong><br>${this.formatMessageHtml(inquiryMessage)}</div>` : ''}
          <p>S pozdravem,<br>Domy v Itálii</p>
        </div>
      `
    });
  }
