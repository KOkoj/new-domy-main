...
// Keep this path fully concrete so file tracing cannot widen to data/import
// (hundreds of MB of source listing images that must stay out of serverless).
...
export async function getLocalProperties() {
  if (hasSupabaseConfig()) {
    ...
    try {
      ...
      // Bundled JSON from git is the source of truth for imported listings
      // (badges, videos, copy). Supabase still contributes admin-only rows.
      return mergeProperties(bundledProperties, supabaseProperties)
    } catch {
...
export async function getLocalPropertyBySlug(slugOrId) {
  if (hasSupabaseConfig()) {
    const bundledProperties = await readPropertiesFromProjectFile()
    const bundled = bundledProperties.find((property) =>
    ...
    try {
      return await readPropertyFromSupabase(slugOrId)
    } catch {
      return null
    }
  }
