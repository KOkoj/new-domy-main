...
const TMP_REGIONS_PATH = path.join(os.tmpdir(), 'local-regions.json')
...
let _writableRegionsPath = null

async function getWritableRegionsPath() {
  if (_writableRegionsPath) return _writableRegionsPath

  const dir = path.dirname(LOCAL_REGIONS_PATH)
  try {
    await fs.mkdir(dir, { recursive: true })
    const handle = await fs.open(LOCAL_REGIONS_PATH, 'a')
    await handle.close()
    _writableRegionsPath = LOCAL_REGIONS_PATH
  } catch {
    _writableRegionsPath = TMP_REGIONS_PATH
  }

  return _writableRegionsPath
}
...
async function ensureDataFile(dataPath) {
  ...
  try {
    ...
    if (dataPath !== LOCAL_REGIONS_PATH) {
      try {
        const seed = await fs.readFile(LOCAL_REGIONS_PATH, 'utf8')
        await fs.writeFile(dataPath, seed, 'utf8')
        return
      } catch {
        // fall through to seeding from page
      }
    }
...
async function readLocalRegions() {
  const dataPath = await getWritableRegionsPath()
  try {
...
async function writeLocalRegions(regions) {
  const dataPath = await getWritableRegionsPath()
  await ensureDataFile(dataPath)
  await fs.writeFile(dataPath, JSON.stringify(regions, null, 2), 'utf8')
}
