...
export const client = createClient({
  ...
  dataset,
  apiVersion,
  useCdn: false, // process.env.NODE_ENV === 'production', // Disabled for debugging
})
...
export const BLOG_ARTICLES_QUERY = `
  *[_type == "article" && articleType == "blog" && !(_id in path("drafts.**"))] | order(date desc) {
    _id,
    slug,
    title,
    excerpt,
...
export const REGION_ARTICLES_QUERY = `
  *[_type == "article" && articleType == "region" && !(_id in path("drafts.**"))] | order(date desc) {
    _id,
    slug,
    title,
    excerpt,
...
export const ARTICLE_BY_SLUG_QUERY = `
  *[_type == "article" && slug == $slug && !(_id in path("drafts.**"))][0] {
    _id,
    slug,
    title,
    excerpt,
