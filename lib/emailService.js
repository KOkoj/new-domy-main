...
Return ONLY a valid JSON object with this exact format:
{
  ...
  async sendEmail({ to, subject, text, html, replyTo }) {
    ...
    // Validate SENDGRID_FROM_EMAIL for Single Sender Verification
    const verifiedSender = process.env.SENDGRID_FROM_EMAIL;
    if (!verifiedSender) {
      const errorMsg = 'Missing SENDGRID_FROM_EMAIL environment variable. SendGrid requires a verified sender.';
    ...
    try {
      const msg = {
        to,
        from: verifiedSender, // STRICTLY enforced system email
        replyTo: replyTo, // Optional: User's email or specific reply-to address
        subject,
        text,
        html
      };
  ...
  async sendFreePdfUpsellEmail({
    ...
    const premiumUrl = `${baseUrl}/premium?product=${encodeURIComponent(premiumProductKey)}`;
    const freePdfTopicByLanguage = {
      'inspections-guide': {
        it: 'sulle visite immobiliari in Italia',
        en: 'for property viewings in Italy',
        cs: 'pro prohlidky nemovitosti v Italii'
      },
      'mistakes-guide': {
        it: 'sugli errori comuni da evitare quando si acquista in Italia',
        en: 'about common mistakes to avoid when buying in Italy',
        cs: 'o castych chybach, kterym je dobre se pri koupi v Italii vyhnout'
      }
    };

    const freePdfTopic = freePdfTopicByLanguage[freePdfKey] || freePdfTopicByLanguage['inspections-guide'];

    const copy = {
      it: {
        subject: 'Grazie per aver scaricato il PDF gratuito',
        title: 'Piu chiarezza oggi, meno sorprese domani',
        greeting: `Ciao ${userName},`,
        intro:
          `Grazie per aver scaricato la nostra guida gratuita ${freePdfTopic.it}.`,
        body:
          "Piu informazioni dettagliate ricevi, piu diventa semplice e trasparente acquistare casa in Italia. Per questo offriamo anche documenti premium con contenuti approfonditi e pratici, pensati per chi e davvero interessato a procedere con metodo.",
        bullets: [
          'controlli essenziali prima dei passaggi vincolanti',
          'spiegazioni chiare su costi, rischi e priorita',
          'struttura operativa concreta, passo dopo passo'
        ],
        cta: 'Scarica il PDF Premium (prezzo ribassato)',
        closing: 'Qui puoi scaricare il PDF Premium a un costo ribassato, quando preferisci.',
        signature: 'Team Domy v Italii'
      },
      en: {
        subject: 'Thank you for downloading the free PDF',
        title: 'More clarity today, fewer surprises tomorrow',
        greeting: `Hi ${userName},`,
        intro:
          `Thank you for downloading our free guide ${freePdfTopic.en}.`,
        body:
          'The more detailed information you have, the easier and more transparent buying a home in Italy becomes. That is why we also offer premium documents with deeper and practical content for people who are genuinely interested in moving forward with a clear method.',
        bullets: [
          'essential checks before binding steps',
          'clear explanations of costs, risks, and priorities',
          'a practical step-by-step structure'
        ],
        cta: 'Download Premium PDF (discounted price)',
        closing: 'You can download the Premium PDF at a discounted price whenever it suits you.',
        signature: 'Domy v Italii Team'
      },
      cs: {
        subject: 'Dekuji, ze jste si stahli PDF zdarma',
        title: 'Vice jasnosti dnes, mene prekvapeni zitra',
        greeting: `Dobry den ${userName},`,
        intro:
          `Dekuji, ze jste si stahli nas bezplatny pruvodce ${freePdfTopic.cs}.`,
        body:
          'Cim vice detailnich informaci mate, tim snazsi a prehlednejsi je koupit nemovitost v Italii. Proto nabizime i premium dokumenty s hlubsim a praktickym obsahem pro ty, kdo to mysli vazne a chteji postupovat systemove.',
        bullets: [
          'zakladni kontroly pred dulezitymi zavaznymi kroky',
          'prehledne vysvetleni nakladu, rizik a priorit',
          'konkretni postup krok za krokem'
        ],
        cta: 'Stahnout Premium PDF (zvyhodnena cena)',
        closing: 'Premium PDF si muzete stahnout za zvyhodnenou cenu kdykoli vam to bude vyhovovat.',
        signature: 'Tym Domy v Italii'
      }
    }[safeLanguage];
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; line-height: 1.6;">
        ...
        <p style="margin: 24px 0;">
          <a href="${premiumUrl}" style="background: #0f172a; color: #fff; padding: 10px 18px; text-decoration: none; border-radius: 6px; display: inline-block;">${copy.cta}</a>
        </p>
        <p>${copy.closing}</p>
        <p>${copy.signature}</p>
      </div>
