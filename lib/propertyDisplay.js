...
// Keep location translations until the selected UI language is known.
export const getPropertyRegionName = (property, language = 'cs') => {
  const city = property?.location?.city
  return getLocalizedValue(city?.region?.name, language, '') ||
    getLocalizedValue(city?.name, language, '') ||
    ({ cs: 'Itálie', it: 'Italia', en: 'Italy' }[language] || 'Italy')
}
export const getPropertyRegionTranslations = (property) =>
  Object.fromEntries(['cs', 'it', 'en'].map(language => [language, getPropertyRegionName(property, language)]))
