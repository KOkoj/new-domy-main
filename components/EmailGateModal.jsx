...
import { LEAD_CONSENT_TEXT } from '@/lib/leadConsent'

export default function EmailGateModal({ source, assetKey, className = '' }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(submitError?.message || 'Odeslání se nepodařilo.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (!authChecked) {
    return (
      <div className={`rounded-xl border border-slate-200 bg-slate-50 p-5 ${className}`}>
        <p className="text-sm text-slate-600">Ověřujeme přihlášení…</p>
      </div>
    )
  }
    ...
    return (
      <div className={`rounded-xl border border-emerald-200 bg-emerald-50/70 p-5 ${className}`}>
        <p className="mb-4 text-sm text-emerald-900">
          Jste přihlášeni v Klubu. PDF si můžete stáhnout bez zadávání e-mailu.
        </p>
  ...
  if (success) {
    return (
      ...
      >
        ...
        <h2 className="text-xl font-bold text-slate-900">Zkontrolujte svůj e-mail</h2>
        <p className="mt-2 text-sm leading-relaxed text-slate-700">
          Poslali jsme vám potvrzovací odkaz. Po potvrzení se PDF automaticky stáhne.
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center gap-2 text-slate-800">
        ...
        <h2 className="text-lg font-bold">Pošlete mi PDF zdarma</h2>
      </div>
      ...
      >
        {submitting ? 'Odesíláme…' : 'Získat PDF zdarma'}
      </Button>
    </form>
  )
}
