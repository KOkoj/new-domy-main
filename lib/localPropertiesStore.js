...
// Resolve the writable path for the local store.
// On read-only filesystems (e.g. Vercel serverless), fall back to /tmp.
// Note: /tmp on Vercel is ephemeral — data does not persist between invocations.
async function resolveDataPath(forWrite = false) {
  if (!forWrite) {
    // For reads, prefer the project data file (may have been seeded at build time)
    try {
      await fs.access(PROJECT_DATA_PATH)
  ...
  // For writes, check if the project data directory is writable
  const dir = path.dirname(PROJECT_DATA_PATH)
  try {
    await fs.mkdir(dir, { recursive: true })
    // Probe writeability with a flag open
...
async function readLocalProperties() {
  const dataPath = await resolveDataPath(false)
  try {
...
async function writeLocalProperties(properties) {
  const dataPath = await resolveDataPath(true)
  await ensureDataFile(dataPath)
