...
import { resolvePropertyType, getPropertyRegionName, getPropertyRegionTranslations } from '@/lib/propertyDisplay'
import { toRegionSlug } from '@/app/properties/filterConfig'

export const getListingRegionName = getPropertyRegionName

export function getListingRegionSlug(property) {
  return (
    property?.location?.city?.region?.slug?.current ||
    toRegionSlug(getListingRegionName(property, 'en'))
  )
}
...
export function transformPropertyListing(property, index = 0) {
  ...
  return {
    ...
    regionI18n: getPropertyRegionTranslations(property),
    ...
    pinnedRank: Number(property?.pinnedRank) || 0,
